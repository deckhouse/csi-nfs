...
{{< alert level="info" >}}
Для работы модуля требуется подключенный модуль [snapshot-controller](../../snapshot-controller/).
{{< /alert >}}
...
Обратите внимание, что управляющие pod csi-драйвера размещаются на узлах кластера согласно суммаризации параметра workloadNodes всех NFSStorageClass. При отсутствии параметра workloadNodes в NFSStorageClass полезная нагрузка будет размещена на всех нодах.
