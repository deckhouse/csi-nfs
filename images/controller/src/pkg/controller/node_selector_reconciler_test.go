...
// IMPORTANT: We assume that controller_suite_test.go provides a NewFakeClient() function.
// Example:
//
...
var _ = Describe(controller.NodeSelectorReconcilerName, func() {
	var (
		...
		// provisionerNFS     = controller.NFSStorageClassProvisioner // "nfs.csi.k8s.io"
	)
	...
	BeforeEach(func() {
		...
		// Use the fake client defined in controller_suite_test.go
		cl = NewFakeClient()
		clusterWideCl = cl
	})
...
`
		...
		// 		It("Scenario 7: Node has a Pod with an NFS PVC -> label is not removed", func() {
		// 			cfgYAML := `
		// nodeSelector:
		//   role: "nfs"
		// `
